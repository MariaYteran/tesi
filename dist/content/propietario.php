...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_propietario = $_POST['cedula_prefix'] . mysqli_real_escape_string($conexion, $_POST['cedula']);
    ...
    if (empty($id_propietario) || empty($nombres) || empty($apellidos) || empty($gmail) || empty($telefono) || empty($password)) {
    ...
    if (empty($error) && !empty($editing_id)) {
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM propietario WHERE id_propietario='$id_propietario' AND id_propietario != '$editing_id' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada por otro propietario.";
            } else {
                if (!empty($password)) {
                    $update = mysqli_query($conexion, "UPDATE propietario SET id_propietario='$id_propietario', nombres='$nombres', apellidos='$apellidos', gmail='$gmail', telefono='$telefono', password='$password' WHERE id_propietario='$editing_id' AND RIF_clinica='$RIF_clinica'");
                } else {
                    $update = mysqli_query($conexion, "UPDATE propietario SET id_propietario='$id_propietario', nombres='$nombres', apellidos='$apellidos', gmail='$gmail', telefono='$telefono' WHERE id_propietario='$editing_id' AND RIF_clinica='$RIF_clinica'");
                }
                if ($update) {
        ...
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM propietario WHERE id_propietario='$id_propietario' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada.";
            } else {
                $insertar = mysqli_query($conexion, "INSERT INTO propietario (id_propietario, nombres, apellidos, gmail, telefono, password, RIF_clinica) VALUES ('$id_propietario', '$nombres', '$apellidos', '$gmail', '$telefono', '$password', '$RIF_clinica')");
                if ($insertar) {
...
if (isset($_GET['editar'])) {
    ...
    $q = mysqli_query($conexion, "SELECT * FROM propietario WHERE id_propietario='$id' AND RIF_clinica='$RIF_clinica'");
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conexion, "DELETE FROM propietario WHERE id_propietario='$id' AND RIF_clinica='$RIF_clinica'");
    exit();
}

$por_pagina = 4;
$pagina = isset($_GET['pagina']) ? max(1, (int)$_GET['pagina']) : 1;
$total_query = mysqli_query($conexion, "SELECT COUNT(*) as total FROM propietario WHERE RIF_clinica='$RIF_clinica'");
$total = mysqli_fetch_assoc($total_query)['total'];
$total_paginas = max(1, ceil($total / $por_pagina));
$pagina = min($pagina, $total_paginas);
$inicio = ($pagina - 1) * $por_pagina;
$query_lista = mysqli_query($conexion, "SELECT * FROM propietario WHERE RIF_clinica='$RIF_clinica' ORDER BY nombres ASC LIMIT $inicio, $por_pagina");
?>
<div class="animate-fadeIn">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-green-800">Propietarios</h1>
            <p class="text-gray-600">Base de datos de clientes y dueños de mascotas.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                ...
                <form id="form-propietario" class="p-5 space-y-4" action="" method="POST">
                    ...
                    <div class="flex gap-2">
                        <button type="submit" id="btn-prop-submit"
                            ...
                            <svg id="btn-prop-icon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                            </svg>
                            <span id="btn-prop-text">Guardar Dueño</span>
                        </button>
                        <button type="button" id="btn-cancel-prop-edit" onclick="cancelarEdicionPropietario()"
        ...
        <div class="lg:col-span-2">
            <div id="prop-card" class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                        ...
                        <span id="prop-count" class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium"><?php echo $total; ?></span>
                    </h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="prop-tbody">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm font-medium"><?php echo $row['id_propietario']; ?></td>
                                ...
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm">
                                    <div class="flex gap-2">
                                        <button onclick="editarPropietario('<?php echo $row['id_propietario']; ?>')"
                                        ...
                                        <button onclick="borrarPropietario('<?php echo $row['id_propietario']; ?>', '<?php echo $row['nombres']; ?>', '<?php echo $row['apellidos']; ?>')"
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm italic text-center" colspan="6">
                                    <div class="flex flex-col items-center py-8">
                                        ...
                                        <p class="text-gray-400 text-sm font-medium">No hay propietarios registrados aún.</p>
                                        <p class="text-gray-400 text-xs mt-1">Utiliza el formulario para registrar un nuevo dueño.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
                ...
                <?php if ($total > 0): ?>
                <div id="prop-paginacion" class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
                    <span class="text-sm text-gray-500">Mostrando <?php echo $inicio + 1; ?> - <?php echo min($inicio + $por_pagina, $total); ?> de <span id="prop-total"><?php echo $total; ?></span></span>
                    <div class="flex items-center gap-2">
                        <?php if ($pagina > 1): ?>
                        <a href="?pagina=<?php echo $pagina - 1; ?>" onclick="event.preventDefault(); cambiarPaginaProp(<?php echo $pagina - 1; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium text-green-700 hover:bg-green-50 border border-green-200 transition-colors">‹</a>
                        <?php endif; ?>
                        <?php for ($p = 1; $p <= $total_paginas; $p++): ?>
                        <a href="?pagina=<?php echo $p; ?>" onclick="event.preventDefault(); cambiarPaginaProp(<?php echo $p; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors border <?php echo $p === $pagina ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white border-green-600' : 'text-green-700 hover:bg-green-50 border-green-200'; ?>"><?php echo $p; ?></a>
                        <?php endfor; ?>
...
<div id="modal-confirmar" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50" style="display:none;">
    <div class="bg-white rounded-2xl shadow-xl w-96 mx-auto overflow-hidden">
        ...
        <div class="px-8 py-8 text-center">
            ...
            <p id="modal-confirmar-msg" class="text-gray-600 text-sm mb-6">¿Estás seguro de borrar este propietario?</p>
...
async function cambiarPaginaProp(p) {
    const res = await fetch(window.location.pathname + '?pagina=' + p);
    const html = await res.text();
    const d = document.createElement('div');
    d.innerHTML = html;
    const tbody = d.querySelector('#prop-tbody');
    const pag = d.querySelector('#prop-paginacion');
    const cnt = d.querySelector('#prop-count');
    const tot = d.querySelector('#prop-total');
    if (tbody) document.querySelector('#prop-tbody').replaceWith(tbody.cloneNode(true));
    if (pag) document.querySelector('#prop-paginacion').replaceWith(pag.cloneNode(true));
    if (cnt) document.querySelector('#prop-count').textContent = cnt.textContent;
    if (tot) document.querySelector('#prop-total').textContent = tot.textContent;
...
window.editarPropietario = async function(id) {
    ...
    const m = data.id_propietario.match(/^([VE]-)(\d+)$/);
    if (m) {
    ...
    document.getElementById('editing_id').value = data.id_propietario;
    document.getElementById('btn-prop-text').textContent = 'Guardar cambios';
    document.getElementById('btn-prop-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />';
    document.getElementById('btn-cancel-prop-edit').classList.remove('hidden');
};

window.cancelarEdicionPropietario = function() {
    document.getElementById('form-propietario').reset();
    document.getElementById('editing_id').value = '';
    document.getElementById('btn-prop-text').textContent = 'Guardar Dueño';
    document.getElementById('btn-prop-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />';
    document.getElementById('btn-cancel-prop-edit').classList.add('hidden');
};
...
window.borrarPropietario = function(id, nombres, apellidos) {
