...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_proveedor = $_POST['cedula_prefix'] . mysqli_real_escape_string($conexion, $_POST['cedula']);
    ...
    if (empty($id_proveedor) || empty($nombres) || empty($apellidos) || empty($telefono) || empty($empresa) || empty($tipo_pago) || empty($cantidad)) {
        $error = "Todos los campos son obligatorios.";
    } elseif (!empty($editing_id)) {
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM proveedor WHERE id_proveedor='$id_proveedor' AND id_proveedor != '$editing_id' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada por otro proveedor.";
            } else {
                $update = mysqli_query($conexion, "UPDATE proveedor SET id_proveedor='$id_proveedor', nombres='$nombres', apellidos='$apellidos', telefono='$telefono', empresa='$empresa', tipo_pago='$tipo_pago', cantidad='$cantidad' WHERE id_proveedor='$editing_id' AND RIF_clinica='$RIF_clinica'");
                if ($update) {
        ...
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM proveedor WHERE id_proveedor='$id_proveedor' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada.";
            } else {
                $insertar = mysqli_query($conexion, "INSERT INTO proveedor (id_proveedor, nombres, apellidos, telefono, empresa, tipo_pago, cantidad, RIF_clinica) VALUES ('$id_proveedor', '$nombres', '$apellidos', '$telefono', '$empresa', '$tipo_pago', '$cantidad', '$RIF_clinica')");
                if ($insertar) {
...
if (isset($_GET['editar'])) {
    ...
    $q = mysqli_query($conexion, "SELECT * FROM proveedor WHERE id_proveedor='$id' AND RIF_clinica='$RIF_clinica'");
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conexion, "DELETE FROM proveedor WHERE id_proveedor='$id' AND RIF_clinica='$RIF_clinica'");
    exit();
}

$por_pagina = 4;
$pagina = isset($_GET['pagina']) ? max(1, (int)$_GET['pagina']) : 1;
$total_query = mysqli_query($conexion, "SELECT COUNT(*) as total FROM proveedor WHERE RIF_clinica='$RIF_clinica'");
$total = mysqli_fetch_assoc($total_query)['total'];
$total_paginas = max(1, ceil($total / $por_pagina));
$pagina = min($pagina, $total_paginas);
$inicio = ($pagina - 1) * $por_pagina;
$query_lista = mysqli_query($conexion, "SELECT * FROM proveedor WHERE RIF_clinica='$RIF_clinica' ORDER BY nombres ASC LIMIT $inicio, $por_pagina");
?>
<div class="animate-fadeIn">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-green-800">Proveedores</h1>
            <p class="text-gray-600">Registra tus proveedores</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                ...
                <form id="form-proveedor" class="p-5 space-y-4" action="" method="POST">
                    ...
                    <div>
                        ...
                        <input type="text" name="empresa" placeholder="Nombre de la empresa" maxlength="25"
                               class="w-full px-4 py-3 border-2 border-green-200 rounded-xl outline-none text-gray-700 placeholder-gray-400 focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Tipo de Pago</label>
                    ...
                    <div>
                        ...
                        <input type="number" name="cantidad" placeholder="0.00" step="0.01" min="0" maxlength="4"
                    ...
                    <div class="flex gap-2">
                        <button type="submit" id="btn-submit"
                            ...
                            <svg id="btn-prove-icon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                            </svg>
                            <span id="btn-prove-text">Guardar Proveedor</span>
                        </button>
                        <button type="button" id="btn-cancel-prove-edit" onclick="cancelarEdicionProveedor()"
        ...
        <div class="lg:col-span-2">
            <div id="proveedor-card" class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                        ...
                        <span id="proveedor-count" class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium"><?php echo $total; ?></span>
                    </h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-green-50">
                                ...
                                <th class="p-4 font-semibold text-green-900 text-sm border-b border-green-100">Empresa</th>
                                <th class="p-4 font-semibold text-green-900 text-sm border-b border-green-100">Tipo de Pago</th>
                        ...
                        <tbody id="proveedor-tbody">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm"><?php echo $row['empresa']; ?></td>
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm"><?php echo $row['tipo_pago']; ?></td>
                                ...
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm">
                                    <div class="flex gap-2">
                                        <button onclick="editarProveedor('<?php echo $row['id_proveedor']; ?>')"
                                        ...
                                        <button onclick="borrarProveedor('<?php echo $row['id_proveedor']; ?>', '<?php echo $row['nombres']; ?>', '<?php echo $row['apellidos']; ?>')"
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm italic text-center" colspan="8">
                                    <div class="flex flex-col items-center py-8">
                                        ...
                                        <p class="text-gray-400 text-sm font-medium">No hay proveedores registrados aún.</p>
                                        <p class="text-gray-400 text-xs mt-1">Utiliza el formulario para registrar un nuevo proveedor.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
                ...
                <?php if ($total > 0): ?>
                <div id="proveedor-paginacion" class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
                    <span class="text-sm text-gray-500">Mostrando <?php echo $inicio + 1; ?> - <?php echo min($inicio + $por_pagina, $total); ?> de <span id="proveedor-total"><?php echo $total; ?></span></span>
                    <div class="flex items-center gap-2">
                        <?php if ($pagina > 1): ?>
                        <a href="?pagina=<?php echo $pagina - 1; ?>" onclick="event.preventDefault(); cambiarPaginaProveedor(<?php echo $pagina - 1; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium text-green-700 hover:bg-green-50 border border-green-200 transition-colors">‹</a>
                        <?php endif; ?>
                        <?php for ($p = 1; $p <= $total_paginas; $p++): ?>
                        <a href="?pagina=<?php echo $p; ?>" onclick="event.preventDefault(); cambiarPaginaProveedor(<?php echo $p; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors border <?php echo $p === $pagina ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white border-green-600' : 'text-green-700 hover:bg-green-50 border-green-200'; ?>"><?php echo $p; ?></a>
                        <?php endfor; ?>
...
<div id="modal-confirmar" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50" style="display:none;">
    <div class="bg-white rounded-2xl shadow-xl w-96 mx-auto overflow-hidden">
        ...
        <div class="px-8 py-8 text-center">
            ...
            <p id="modal-confirmar-msg" class="text-gray-600 text-sm mb-6">¿Estás seguro de borrar este proveedor?</p>
...
async function cambiarPaginaProveedor(p) {
    const res = await fetch(window.location.pathname + '?pagina=' + p);
    const html = await res.text();
    const d = document.createElement('div');
    d.innerHTML = html;
    const tbody = d.querySelector('#proveedor-tbody');
    const pag = d.querySelector('#proveedor-paginacion');
    const cnt = d.querySelector('#proveedor-count');
    const tot = d.querySelector('#proveedor-total');
    if (tbody) document.querySelector('#proveedor-tbody').replaceWith(tbody.cloneNode(true));
    if (pag) document.querySelector('#proveedor-paginacion').replaceWith(pag.cloneNode(true));
    if (cnt) document.querySelector('#proveedor-count').textContent = cnt.textContent;
    if (tot) document.querySelector('#proveedor-total').textContent = tot.textContent;
    const card = document.querySelector('#proveedor-card');
    if (card) {
...
window.editarProveedor = async function(id) {
    ...
    const m = data.id_proveedor.match(/^([VE]-)(\d+)$/);
    if (m) {
    ...
    document.querySelector('input[name="empresa"]').value = data.empresa;
    document.querySelector('select[name="tipo_pago"]').value = data.tipo_pago;
    document.querySelector('input[name="cantidad"]').value = data.cantidad;

    document.getElementById('editing_id').value = data.id_proveedor;
    document.getElementById('btn-prove-text').textContent = 'Guardar cambios';
    document.getElementById('btn-prove-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />';
    document.getElementById('btn-cancel-prove-edit').classList.remove('hidden');
};

window.cancelarEdicionProveedor = function() {
    document.getElementById('form-proveedor').reset();
    document.getElementById('editing_id').value = '';
    document.getElementById('btn-prove-text').textContent = 'Guardar Proveedor';
    document.getElementById('btn-prove-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />';
    document.getElementById('btn-cancel-prove-edit').classList.add('hidden');
};
...
window.borrarProveedor = function(id, nombres, apellidos) {
