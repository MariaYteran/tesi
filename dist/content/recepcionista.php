...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_recepcionista = $_POST['cedula_prefix'] . mysqli_real_escape_string($conexion, $_POST['cedula']);
    ...
    if (empty($id_recepcionista) || empty($nombres) || empty($apellidos) || empty($gmail) || empty($telefono) || empty($password)) {
    ...
    if (empty($error) && !empty($editing_id)) {
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM recepcionista WHERE id_recepcionista='$id_recepcionista' AND id_recepcionista != '$editing_id' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada por otro recepcionista.";
            } else {
                if (!empty($password)) {
                    $update = mysqli_query($conexion, "UPDATE recepcionista SET id_recepcionista='$id_recepcionista', nombres='$nombres', apellidos='$apellidos', gmail='$gmail', telefono='$telefono', password='$password' WHERE id_recepcionista='$editing_id' AND RIF_clinica='$RIF_clinica'");
                } else {
                    $update = mysqli_query($conexion, "UPDATE recepcionista SET id_recepcionista='$id_recepcionista', nombres='$nombres', apellidos='$apellidos', gmail='$gmail', telefono='$telefono' WHERE id_recepcionista='$editing_id' AND RIF_clinica='$RIF_clinica'");
                }
                if ($update) {
                    $success = "Recepcionista actualizado exitosamente.";
                } else {
        ...
        try {
            $verificar = mysqli_query($conexion, "SELECT * FROM recepcionista WHERE id_recepcionista='$id_recepcionista' AND RIF_clinica='$RIF_clinica'");
            if (mysqli_num_rows($verificar) > 0) {
                $error = "La cédula ya está registrada.";
            } else {
                $insertar = mysqli_query($conexion, "INSERT INTO recepcionista (id_recepcionista, nombres, apellidos, gmail, telefono, password, RIF_clinica) VALUES ('$id_recepcionista', '$nombres', '$apellidos', '$gmail', '$telefono', '$password', '$RIF_clinica')");
                if ($insertar) {
                    $success = "Recepcionista registrado exitosamente.";
                } else {
...
if (isset($_GET['editar'])) {
    ...
    $q = mysqli_query($conexion, "SELECT * FROM recepcionista WHERE id_recepcionista='$id' AND RIF_clinica='$RIF_clinica'");
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conexion, "DELETE FROM recepcionista WHERE id_recepcionista='$id' AND RIF_clinica='$RIF_clinica'");
    exit();
}

$por_pagina = 4;
$pagina = isset($_GET['pagina']) ? max(1, (int)$_GET['pagina']) : 1;
$total_query = mysqli_query($conexion, "SELECT COUNT(*) as total FROM recepcionista WHERE RIF_clinica='$RIF_clinica'");
$total = mysqli_fetch_assoc($total_query)['total'];
$total_paginas = max(1, ceil($total / $por_pagina));
$pagina = min($pagina, $total_paginas);
$inicio = ($pagina - 1) * $por_pagina;
$query_lista = mysqli_query($conexion, "SELECT * FROM recepcionista WHERE RIF_clinica='$RIF_clinica' ORDER BY nombres ASC LIMIT $inicio, $por_pagina");
?>
<div class="animate-fadeIn">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-green-800">Recepcionistas</h1>
            <p class="text-gray-600">Gestión del personal de atención.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                ...
                <form id="form-recepcionista" class="p-5 space-y-4" action="" method="POST">
                    ...
                    <div>
                        ...
                        <div class="flex">
                            ...
                            <input type="text" name="cedula" placeholder="12345678" maxlength="25"
                    ...
                    <div class="flex gap-2">
                        <button type="submit" id="btn-recep-submit"
                            ...
                            <svg id="btn-recep-icon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                            </svg>
                            <span id="btn-recep-text">Guardar Recepcionista</span>
                        </button>
                        <button type="button" id="btn-cancel-recep-edit" onclick="cancelarEdicionRecepcionista()"
        ...
        <div class="lg:col-span-2">
            <div id="recep-card" class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                        ...
                        <span id="recep-count" class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium"><?php echo $total; ?></span>
                    </h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="recep-tbody">
                            <?php if ($total > 0): while ($row = mysqli_fetch_assoc($query_lista)): ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm font-medium"><?php echo $row['id_recepcionista']; ?></td>
                                ...
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm">
                                    <div class="flex gap-2">
                                        <button onclick="editarRecepcionista('<?php echo $row['id_recepcionista']; ?>')"
                                        ...
                                        <button onclick="borrarRecepcionista('<?php echo $row['id_recepcionista']; ?>', '<?php echo $row['nombres']; ?>', '<?php echo $row['apellidos']; ?>')"
                            ...
                            <?php endwhile; else: ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm italic text-center" colspan="5">
                                    <div class="flex flex-col items-center py-8">
                                        <svg class="w-12 h-12 text-gray-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                        <p class="text-gray-400 text-sm font-medium">No hay recepcionistas registrados aún.</p>
                                        <p class="text-gray-400 text-xs mt-1">Utiliza el formulario para registrar un nuevo recepcionista.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
                ...
                <?php if ($total > 0): ?>
                <div id="recep-paginacion" class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
                    <span class="text-sm text-gray-500">Mostrando <?php echo $inicio + 1; ?> - <?php echo min($inicio + $por_pagina, $total); ?> de <span id="recep-total"><?php echo $total; ?></span></span>
                    <div class="flex items-center gap-2">
                        <?php if ($pagina > 1): ?>
                        <a href="?pagina=<?php echo $pagina - 1; ?>" onclick="event.preventDefault(); cambiarPaginaRecep(<?php echo $pagina - 1; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium text-green-700 hover:bg-green-50 border border-green-200 transition-colors">‹</a>
                        <?php endif; ?>
                        <?php for ($p = 1; $p <= $total_paginas; $p++): ?>
                        <a href="?pagina=<?php echo $p; ?>" onclick="event.preventDefault(); cambiarPaginaRecep(<?php echo $p; ?>)" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors border <?php echo $p === $pagina ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white border-green-600' : 'text-green-700 hover:bg-green-50 border-green-200'; ?>"><?php echo $p; ?></a>
                        <?php endfor; ?>
...
<div id="modal-confirmar" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50" style="display:none;">
    <div class="bg-white rounded-2xl shadow-xl w-96 mx-auto overflow-hidden">
        ...
        <div class="px-8 py-8 text-center">
            ...
            <p id="modal-confirmar-msg" class="text-gray-600 text-sm mb-6">¿Estás seguro de borrar este recepcionista?</p>
...
<script>
document.getElementById('form-recepcionista').addEventListener('submit', async function(e) {
    e.preventDefault();
...
async function cambiarPaginaRecep(p) {
    const res = await fetch(window.location.pathname + '?pagina=' + p);
    const html = await res.text();
    const d = document.createElement('div');
    d.innerHTML = html;
    const tbody = d.querySelector('#recep-tbody');
    const pag = d.querySelector('#recep-paginacion');
    const cnt = d.querySelector('#recep-count');
    const tot = d.querySelector('#recep-total');
    if (tbody) document.querySelector('#recep-tbody').replaceWith(tbody.cloneNode(true));
    if (pag) document.querySelector('#recep-paginacion').replaceWith(pag.cloneNode(true));
    if (cnt) document.querySelector('#recep-count').textContent = cnt.textContent;
    if (tot) document.querySelector('#recep-total').textContent = tot.textContent;
    const card = document.querySelector('#recep-card');
    if (card) {
...
window.editarRecepcionista = async function(id) {
    const res = await fetch(window.location.pathname + '?editar=' + encodeURIComponent(id));
    const data = await res.json();

    const m = data.id_recepcionista.match(/^([VE]-)(\d+)$/);
    if (m) {
    ...
    document.getElementById('editing_id').value = data.id_recepcionista;
    document.getElementById('btn-recep-text').textContent = 'Guardar cambios';
    document.getElementById('btn-recep-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />';
    document.getElementById('btn-cancel-recep-edit').classList.remove('hidden');
};

window.cancelarEdicionRecepcionista = function() {
    document.getElementById('form-recepcionista').reset();
    document.getElementById('editing_id').value = '';
    document.getElementById('btn-recep-text').textContent = 'Guardar Recepcionista';
    document.getElementById('btn-recep-icon').innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />';
    document.getElementById('btn-cancel-recep-edit').classList.add('hidden');
};
...
window.borrarRecepcionista = function(id, nombres, apellidos) {
