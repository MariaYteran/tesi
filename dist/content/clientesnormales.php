...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_propietario) || empty($nombres) || empty($apellidos) || empty($telefono)) {
        $error = "Todos los campos son obligatorios.";
    } elseif (!empty($editing_id)) {
        try {
            $update = mysqli_query($conexion, "UPDATE clientenormal SET id_propietario='$id_propietario', nombres='$nombres', apellidos='$apellidos', telefono='$telefono' WHERE id_propietario='$editing_id' AND RIF_clinica='$RIF_clinica'");
            if ($update) {
        ...
        $verificar = mysqli_query($conexion, "SELECT * FROM clientenormal WHERE id_propietario='$id_propietario' AND RIF_clinica='$RIF_clinica'");
        if (mysqli_num_rows($verificar) > 0) {
            $error = "La cédula ya está registrada.";
        } else {
            try {
                $insertar = mysqli_query($conexion, "INSERT INTO clientenormal (id_propietario, nombres, apellidos, telefono, RIF_clinica) VALUES ('$id_propietario', '$nombres', '$apellidos', '$telefono', '$RIF_clinica')");
                if ($insertar) {
...
if (isset($_GET['editar'])) {
    ...
    $q = mysqli_query($conexion, "SELECT * FROM clientenormal WHERE id_propietario='$id' AND RIF_clinica='$RIF_clinica'");
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conexion, "DELETE FROM clientenormal WHERE id_propietario='$id' AND RIF_clinica='$RIF_clinica'");
    exit();
}

$por_pagina = 4;
$pagina = isset($_GET['pagina']) ? max(1, (int)$_GET['pagina']) : 1;
$total_query = mysqli_query($conexion, "SELECT COUNT(*) as total FROM clientenormal WHERE RIF_clinica='$RIF_clinica'");
$total = mysqli_fetch_assoc($total_query)['total'];
$total_paginas = max(1, ceil($total / $por_pagina));
if ($pagina > $total_paginas) $pagina = $total_paginas;
$inicio = ($pagina - 1) * $por_pagina;
$query_lista = mysqli_query($conexion, "SELECT * FROM clientenormal WHERE RIF_clinica='$RIF_clinica' ORDER BY nombres ASC LIMIT $por_pagina OFFSET $inicio");
?>
<div class="animate-fadeIn">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-green-800">Clientes</h1>
            <p class="text-gray-600">Registro y lista de clientes varios</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                ...
                <form id="form-cliente" class="p-5 space-y-4" action="" method="POST">
                    <div>
                    ...
                    <button type="submit" id="btn-submit-cliente"
        ...
        <div class="lg:col-span-2">
            <div id="cliente-card" class="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <h2 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                            ...
                            Lista de Clientes
                            <span id="cliente-count" class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium"><?php echo $total; ?></span>
                        </h2>
                        ...
                        <tbody id="cliente-tbody">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="p-4 border-b border-gray-100 whitespace-nowrap">
                                    <button onclick="editarCliente('<?php echo $row['id_propietario']; ?>')" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:from-green-700 hover:to-emerald-700 transition-all inline-block cursor-pointer">Editar</button>
                                    <button onclick="borrarCliente('<?php echo $row['id_propietario']; ?>', '<?php echo addslashes($row['nombres']); ?>', '<?php echo addslashes($row['apellidos']); ?>')" class="bg-red-500 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-red-600 transition-all inline-block ml-1.5 cursor-pointer">Borrar</button>
                                </td>
                            </tr>
                            <?php endwhile; else: ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 border-b border-gray-100 text-gray-700 text-sm italic text-center" colspan="5">
                                    <div class="flex flex-col items-center py-8">
                                        ...
                                        <p class="text-gray-400 text-sm font-medium">No hay clientes registrados aún.</p>
                                        <p class="text-gray-400 text-xs mt-1">Utiliza el formulario para registrar un nuevo cliente.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
                ...
                <?php if ($total > 0): ?>
                <div id="cliente-paginacion" class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
...
<div id="modal-confirmar" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50" style="display:none;">
    <div class="bg-white rounded-2xl shadow-xl w-96 mx-auto overflow-hidden">
        ...
        <div class="px-8 py-8 text-center">
            ...
            <p id="modal-confirmar-msg" class="text-gray-600 text-sm mb-6">¿Estás seguro de borrar este cliente?</p>
...
async function cambiarPagina(p) {
    ...
    d.innerHTML = html;
    const tbody = d.querySelector('#cliente-tbody');
    const pag = d.querySelector('#cliente-paginacion');
    const cnt = d.querySelector('#cliente-count');
    const idx = d.querySelector('#page-indicator');
    if (tbody) document.querySelector('#cliente-tbody').replaceWith(tbody.cloneNode(true));
    if (pag) document.querySelector('#cliente-paginacion').replaceWith(pag.cloneNode(true));
    if (cnt) document.querySelector('#cliente-count').textContent = cnt.textContent;
...
window.borrarCliente = function(id, nombres, apellidos) {
